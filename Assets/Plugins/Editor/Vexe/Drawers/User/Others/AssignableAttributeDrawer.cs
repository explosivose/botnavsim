...
namespace Vexe.Editor.Drawers
{
	...
	{
		private string kFoldout, kTarget, kMember, kSource;
		...
		{
			var iden = id + attribute.GetType().Name;
			kSource  = iden + "src";
			kFoldout = iden + "foldout";
			kTarget  = iden + "target";
			kMember  = iden + "member";
					...
					if (!string.IsNullOrEmpty(targetTypeName))
					{
						...
						if (target != null)
						{
							...
							{ 
								...
								targetMember = targetType.Member(memberName, Flags.InstancePublic);
						...
						if (members.Count > 0)
						{
							...
							if (targetMember != null)
							{
								...
								if (targetMember.MemberType == MemberTypes.Field)
									result = (targetMember as FieldInfo).DelegateForGetFieldValue().Invoke(target);
								else if (targetMember.MemberType == MemberTypes.Property)
									result = (targetMember as PropertyInfo).DelegateForGetPropertyValue().Invoke(target);
								else result = (targetMember as MethodInfo).DelegateForCallMethod().Invoke(target);
